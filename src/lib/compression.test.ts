...
describe("compression helpers", () => {
  ...
  it("detects image formats from MIME types and extensions", () => {
    expect(imageFormat(new File([], "photo.jpg", { type: "image/jpeg" }))).toBe(
      "jpeg",
    );
  ...
  it("detects supported document and video formats", () => {
    expect(
      fileFormat(new File([], "document.pdf", { type: "application/pdf" })),
    ).toBe("pdf");
    expect(fileFormat(new File([], "clip.mp4", { type: "video/mp4" }))).toBe(
      "mp4",
    );
  });
});
