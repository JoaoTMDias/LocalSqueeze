...
async function compressImage(
  ...
  if (format === "jpeg")
    return {
      buffer: outputBuffer(
        new Uint8Array(await encodeJpeg(imageData, { quality })),
      ),
      mimeType,
    };
  if (format === "png")
    return {
      buffer: outputBuffer(new Uint8Array(await optimisePng(imageData))),
      mimeType,
    };
...
function validateSvg(svg: string) {
  if (!/<svg(?:\s|>)/i.test(svg))
    throw new Error("The file does not contain a valid SVG root.");
  if (
    /<(?:script|foreignObject|iframe|object|embed)\b/i.test(svg) ||
    /\bon[a-z]+\s*=/i.test(svg) ||
    /(?:javascript|vbscript):/i.test(svg) ||
    /<!doctype/i.test(svg)
  ) {
    throw new Error(
      "This SVG contains unsupported or potentially unsafe content.",
    );
  }
}
...
async function compressSvg(
  ...
  const result = optimize(source, {
    plugins: [
      presetConfig,
      ...(options.preserveMetadata ? [] : ["removeTitle" as const]),
    ],
  });
  postProgress(id, 80);
  ...
  return {
    buffer: outputBuffer(
      optimized.byteLength < input.byteLength ? optimized : input,
    ),
...
async function compressMp4(id: string, file: ArrayBuffer, scale: number) {
  ...
  const onProgress = ({ progress }: { progress: number }) =>
    postProgress(
      id,
      Math.min(95, Math.max(10, Math.round(progress * 85 + 10))),
    );
...
worker.onmessage = async ({ data }: MessageEvent<CompressRequest>) => {
  try {
    ...
    worker.postMessage(
      { type: "complete", id: data.id, ...result } satisfies WorkerResponse,
