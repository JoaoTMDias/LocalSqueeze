...
test.describe("LocalSqueeze compression workflow", () => {
  test("compresses a PNG in the production browser build", async ({ page }) => {
    ...
    await expect(
      page.locator("p.truncate", { hasText: "smoke.png" }),
    ).toBeVisible();
    await expect
      .poll(
        async () => {
    ...
    await expect(
      page.getByRole("button", { name: "Download smoke.png" }),
    ).toBeEnabled();
    expect(progressValues.length).toBeGreaterThan(0);
  });

  test("keeps advanced settings optional and exposes scale choices", async ({
    page,
  }) => {
    await page.goto("/");
